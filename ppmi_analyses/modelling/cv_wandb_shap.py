...
if __name__ == '__main__':
    ...
    for fold, (train_msk, test_msk, val_msk) in enumerate(zip(*k_fold(X, y, folds))):
        ...
        display_graph(fold, G, pos, labels_dict, save_fig=False, path=OUT_DIR,
                        ...
                        plot_title=sweep_run.name + "- network - fold " + str(fold), wandb_log=False)
        ...
        if "MLP" in model._get_name():
            ...
            # feature importance
            feature_importance = feature_importances_shap_values(model, data, X_processed, device, names_list=feat_names, n=20, save_fig=True, name_file=f'{sweep_run.name}-{fold}_feature_importance',path=OUT_DIR)
