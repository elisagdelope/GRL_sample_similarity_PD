# Analogous to cv_wandb.py but for a random network
import numpy as np
import pandas as pd
import random
import os
...
if __name__ == '__main__':
    ...
    OUT_DIR = OUT_DIR + "rnet_" + myconfig.model_name + "_" + datestamp + "/" # change vs cv_wandb.py
    ...
    for fold, (train_msk, test_msk, val_msk) in enumerate(zip(*k_fold(X, y, folds))):
        ...
        # build network
        adj = random_network(edge_percentage=myconfig.edge_percent, X_df=X_processed)  # change vs cv_wandb.py
