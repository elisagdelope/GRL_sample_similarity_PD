...
sweep_config = {'project': 'my-cv-test5',
               ...
               'parameters': {'C_lasso': {'values': [0.1, 0.05]},
                              ...
                              'edge_percent': {'value': 0.1},
                              'model_name': {'value': "GINE"},
                              'n_epochs': {'value': 50},
                              'lr': {'value': 0.03},
                              'lrscheduler_factor': {'value': 0.9},
                              'dropout': {'value': 0.5},
                              'weight_decay': {'value': 0.01},
                              'h1_hidden_units': {'value': 8},
                              'h2_hidden_units': {'value': 8},
                              ...
                              'heads': {'value':3}}}
wandb.login()
sweep_id = wandb.sweep(sweep=sweep_config, project='my-cv-test5', entity='elisa-gomezdelope') # set up environment variable??
...
def main():
    ...
    # set up wandb
    sweep_run = wandb.init(config=sweep_config)
