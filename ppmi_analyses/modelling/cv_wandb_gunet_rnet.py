# Analogous to cv_wandb.py but does not extract feature importance information (gnnexplainer does not support GUNet at this time)
import numpy as np
import pandas as pd
import random
import os
...
if __name__ == '__main__':
    ...
    OUT_DIR = OUT_DIR + "rnet_" + myconfig.model_name + "_" + datestamp + "/" # change vs cv_wandb_gunet.py
    ...
    for fold, (train_msk, test_msk, val_msk) in enumerate(zip(*k_fold(X, y, folds))):
        ...
        # build network
        adj = random_network(edge_percentage=myconfig.edge_percent, X_df=X_processed)  # change vs cv_wandb_gunet.py
        ...
        # feature importance: dummy list for now
        feature_importance = [f'dummy{i}' for i in range(5)]
